...
def generate_audio(text, voice_id):
    ...
    json_file1 = os.path.join(json_dir, "chemistry_news_podcast.json")
    json_file2 = os.path.join(json_dir, "chemistry_news_podcast_part2.json")
...
def main():
    ...
    try:
        ...
        print(f"Error: {e}")
        print("Please run create_chemistry_news_podcast.py and create_chemistry_news_podcast_part2.py first")
        return
    
    # Check if final output already exists
    output_file = os.path.join(output_folder, "chemistry-news-episode1.mp3")
    ...
    part1_file = os.path.join(output_folder, "chemistry-news-part1.mp3")
    if os.path.exists(part1_file):
        ...
        audio_part1 = process_podcast_part(podcast_part1, output_folder, "part1")
    ...
    part2_file = os.path.join(output_folder, "chemistry-news-part2.mp3")
    if os.path.exists(part2_file):
        ...
        audio_part2 = process_podcast_part(podcast_part2, output_folder, "part2")
        audio_part2.export(part2_file, format="mp3", bitrate="192k")
        print(f"Part 2 completed and saved to {part2_file}")
    
    # Combine the parts
    print("Combining parts into final podcast...")
    try:
        ...
        try:
            ...
            import subprocess
            cmd = f"ffmpeg -y -i {part1_file} -i {part2_file} -filter_complex '[0:0][1:0]concat=n=2:v=0:a=1[out]' -map '[out]' {output_file}"
