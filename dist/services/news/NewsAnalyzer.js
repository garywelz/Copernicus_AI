...
class NewsAnalyzer {
     ...
     */
    async analyzeArticles(articles, options = {}) {
        try {
            // Group articles by subtopic
            const groupedArticles = await this.groupArticlesByTopic(articles);
            // Analyze each group
            const analyses = await Promise.all(Object.values(groupedArticles).map(group => this.analyzeArticleGroup(group, options)));
...
`).join('\n')}`;
            ...
            return Object.fromEntries(Object.entries(groups).map(([topic, indices]) => [
                topic,
     ...
     */
    async analyzeArticleGroup(articles, options) {
     ...
     */
    async findConnections(analyses) {
        try {
     ...
     */
    constructAnalysisPrompt(articles, format) {
        return `
...
Please provide the analysis in the following JSON format:
{
     ...
     */
    constructConnectionsPrompt(analyses) {
        return `
...
exports.NewsAnalyzer = NewsAnalyzer;
